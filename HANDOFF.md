...
- **2026-08-03**: Added `money-engine/INSTRUCTIONS.md` — canonical copy of the 12-Hour Money Engine project instructions, then hardened it. Now carries a Non-negotiables block (automatic delivery, merchant-of-record / VAT, captured-vs-settled cash timing, written refund policy, no approval-gated launches), a three-tier evidence rule (checkable → `VERIFY`, volatile → check live, unknowable → labeled `ASSUMPTION` + falsifier), a "Never propose" list, and `/triage` + `/verify` commands. Repo copy is the source of truth; re-paste into the Claude project after any edit. **Two open items for Isaac:** the "My assets" line still needs real handles and follower counts, and the Amazon monetization stack file it references is not in this repo — it must be uploaded to the Claude project directly.
