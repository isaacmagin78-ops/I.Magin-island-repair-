...
export default function Timeline({ state, onAddReminder, onRemoveReminder, onToggleTask }: TimelineProps) {
  ...
  const daysToMoveIn = Math.max(
    0,
    Math.ceil((moveIn.getTime() - Date.now()) / (1000 * 60 * 60 * 24)),
  );
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <div className="bg-gradient-to-br from-ink to-ink-secondary p-6 text-cream flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold">Your College Journey</h1>
            <p className="text-cream/80 text-sm mt-1">
              {state.profile.studentName} · Class of {state.profile.graduationYear}
            </p>
          </div>
          <div className="text-center bg-white/10 rounded-xl px-5 py-3 border border-white/15">
            <p className="text-3xl font-extrabold text-gold-light">{daysToMoveIn}</p>
            <p className="text-xs text-cream/80">days to move-in</p>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-3">
        {PHASE_META.map((meta) => {
          ...
          return (
            <Card
              key={meta.id}
              className={`overflow-hidden ${isCurrent ? 'ring-2 ring-gold' : ''} ${isPast ? 'opacity-80' : ''}`}
            >
              <button
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isCurrent && (
                      <span className="text-xs font-bold text-ink bg-gold px-2 py-0.5 rounded-full">You are here</span>
                    )}
                  </div>
                  <p className="text-xs text-ink-secondary">{meta.subtitle}</p>
                  {tasks.length > 0 && (
                    <div className="mt-2 max-w-xs">
                      <Bar percent={percent} tone={percent >= 100 ? 'green' : percent > 0 ? 'blue' : 'gray'} />
                    </div>
                  )}
                </div>
                ...
                <div className="px-4 pb-4 border-t border-slate-100">
                  <div className="pt-3">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {meta.goals.map((g) => (
                        <span key={g} className="text-xs px-2 py-1 rounded bg-slate-100 text-slate-600">
                          {g}
                        </span>
                      ))}
                    </div>
                    ...
                    {tasks.length > 0 ? (
                      <div className="space-y-2">
                        {tasks.map((t) => (
                          <TaskRow key={t.id} task={t} state={state} onToggleTask={onToggleTask} />
                        ))}
                      </div>
                    ) : (
                      <p className="text-sm text-ink-secondary">
                        {isPast ? 'This phase is behind you.' : 'Tasks for this phase will appear here as they come up.'}
                      </p>
                    )}
                  </div>
                </div>
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-bold text-ink">Custom reminders</h2>
          <button
          ...
          <div className="space-y-2">
            {state.reminders
              ...
              .map((r) => (
                <div
                  key={r.id}
                  className="flex items-start justify-between p-3 rounded-xl border border-slate-100 bg-slate-50"
                >
                  <div>
                    <p className="font-semibold text-ink">{r.title}</p>
                    <p className="text-xs text-ink-secondary mt-0.5">
                      {formatDate(r.date)} · {ownerLabel(r.assignedTo)}
                    </p>
                    {r.notes && <p className="text-sm text-ink-secondary italic mt-1">{r.notes}</p>}
                  </div>
                  <button
...
function TaskRow({
  ...
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-100 hover:bg-slate-50 transition">
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium text-ink truncate ${task.completed ? 'line-through text-slate-400' : ''}`}>
          {task.title}
        </p>
        {due && <p className="text-xs text-ink-secondary">{relativeDue(due)}</p>}
      </div>
  ...
  return (
    <form onSubmit={submit} className="space-y-3 mb-4 p-4 bg-slate-50 rounded-xl">
      ...
      <div className="flex gap-2">
        <button type="submit" className="flex-1 bg-gold text-ink py-2 rounded-lg font-semibold hover:bg-gold-light transition">
          Add reminder
        </button>
        <button
          type="button"
